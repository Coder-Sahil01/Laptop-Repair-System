...
void addlaptop()
{
	...
	for(i=0;i<n;i++)
	{
		...
		fflush(stdin);
		gets(lap.model);
		printf("Describe Problem in Laptop in few word:");
		fflush(stdin);
		gets(lap.problem);
...
void search()
{
	...
	else
	{
		...
		while(fread(&lap,sizeof(lap),1,ft))
		{
			if(x==lap.id)
			{
				printf("\nLaptop-details:-\n\tBrand-name:%s\n\tModel:%s\n\tInitial-price:%d\n\tProblem:%s",lap.brand,lap.model,lap.price,lap.problem);
			}
		}
		fclose(ft);
	}
...
int main()
{
	...
	do
	{
		...
		printf("Press 7 to search:\n");
		printf("Press 8 to find id by mobileno:\n");
